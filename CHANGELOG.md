...
- 3D lake-bed view — a new **View** selector in the header (beside the base-map picker) switches the active scan between the classic 2D overlays and an explorable, depth-coloured 3D model of the lake bed. The surface is built from the same interpolated depth grid that already drives the bathymetry contours, so no extra data is needed; it's coloured with the same viridis_r ramp (deeper = darker) and lit so drop-offs and channels stand out. Drag to pan, right-drag (or ctrl-drag) to tilt and rotate around it. A **Vertical exaggeration** slider (1×–30×, default 6×) stretches the relief — essential because lakes are near-flat in true scale — and updates the surface instantly. Works for depth-only mobile scans too. Renders in the map's own WebGL context, so it stays a single self-contained `index.html` with no new dependencies. The view choice and exaggeration persist across sessions.

...
- 3D lake-bed view — a single **View** selector in the header (3D Model / 2D OpenStreetMap / 2D Satellite) switches the active scan between the classic 2D overlays and an explorable, depth-coloured 3D model of the lake bed. 3D Model is a standalone view: the basemap is dropped entirely and the surface sits on a clean backdrop, so there's no map to align to and no distracting tiles. The surface is built from the same interpolated depth grid that already drives the bathymetry contours, so no extra data is needed; it's coloured with the same viridis_r ramp (deeper = darker) and lit so drop-offs and channels stand out. Drag to pan, pinch to zoom, and use the **Tilt** slider (20°–80°, or a two-finger drag) to look across it — plus a **Reset view** button to re-frame and level the camera. The panel also carries the depth colour key, and the 2D-only layer controls (weed, fish density, sweet spots, temperature) are disabled while in 3D since the model shows depth alone.
- Compass — a small compass over both the 2D and 3D views shows the current map orientation (its needle points north as you rotate); click it to snap back to north-up. A **Vertical exaggeration** slider (1×–20×, default 6×) stretches the relief — essential because lakes are near-flat in true scale — and updates the surface instantly. Works for depth-only mobile scans too. Renders in the map's own WebGL context (with relative-precision coordinates so it stays crisp at any zoom), so it's a single self-contained `index.html` with no new dependencies. The view choice and exaggeration persist across sessions.
